...
Insights
Settings
sequential_sentence_classification/sequential_sentence_classification/predictor.py /
@Mahasweta-usc
Mahasweta-usc Update predictor.py
…
Latest commit 0bb114c 1 hour ago
 History
 1 contributor
246 lines (203 sloc)  7.39 KB
...
def segment_text(chunk,url):
	global candidate, email_sent
	email_sent[url] = [sent_elem for sent_elem in sent_break(process_(chunk["last_reply"]))] 
	candidate[url] = []
...
def process_text( filename):
	...
	for _ ,row in outtable.iterrows():
		if not i%1000: print("Segmented {}/{} emails".format(i,f.shape[0])) 
		results.append(deadpool.apply_async(segment_text, args=(row,row["url"])))
