...
def segment_text(chunk,url,current,nlp):
	global candidate, email_sent
	email_sent[url] = [sent_elem for sent_elem in sent_break(process_(chunk["last_reply"]),nlp)] 
	candidate[url] = []
...
def process_text( filename):
	...
	for i ,row in f.iterrows():
		results.append(deadpool.apply_async(segment_text, args=(row,row["url"],i,nlp)))
